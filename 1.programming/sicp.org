...
- A name identifies a "variable" whose "value" *is* the computational
  ...
  :personal-note:
  Its worth noting that just as in real life, the word is no the thing
  (the word "dog" is not the dog), its just used to refer to it.
...
- We should ask ourselves, what means do any specific language
  provides for naming computational objects objects.
